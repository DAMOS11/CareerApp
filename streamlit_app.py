...
def load_data():
    ...
    df["Combined_Text"] = df["Education"] + \
        " " + df["Skills"] + " " + df["Interests"]
...
def parse_resume_info(text):
    ...
    education_keywords = ["bachelor", "master",
                          "msc", "bsc", "phd", "degree", "diploma"]
    interest_keywords = ["ai", "technology",
                         "design", "media", "healthcare", "management"]

    education = next((word.title()
                     for word in education_keywords if word in text), "Not Found")
...
def recommend_top_3_with_resources(education, skills, interests):
    ...
    top_careers = [(label_encoder.inverse_transform([i])[0], round(
        probabilities[i]*100, 2)) for i in top_indices]
...
if option == "Upload Resume":
    ...
    if uploaded_file:
        ...
        if st.button("Get Recommendations"):
            st.markdown(recommend_top_3_with_resources(
                education, skills, interests))

else:
    education = st.text_input("Education", "Bachelor's in Computer Science")
    skills = st.text_area("Skills (semicolon-separated)",
                          "Python; Machine Learning; Data Analysis")
    interests = st.text_area(
        "Interests (semicolon-separated)", "AI; Technology")
    if st.button("Get Recommendations"):
        st.markdown(recommend_top_3_with_resources(
            education, skills, interests))
